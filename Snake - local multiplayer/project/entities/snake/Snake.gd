...
const color_pairs = [[Color("#fffa66"), Color("#66ff66")], [Color("#7966ff"), Color("#6699ff")]]

# Called when the node enters the scene tree for the first time.
func _ready():
	$Sprite.self_modulate = color_pairs[self.player][0]
	add_to_group("Player " + str(self.player))
	$PlayerInputComponent.player = self.player
	$PlayerInputComponent.connect("input_detected", self, "_on_PlayerInputComponent_input_detected")
	emit_signal("size_changed", body_segments.size(), self.player)
...
func eat_food(value) -> void:
	for i in range(value):
		...
		tail_segment.get_child(0).self_modulate = color_pairs[self.player][1]
		tail_segment.add_to_group("Player " + str(self.player))
